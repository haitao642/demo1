...
namespace Deepleo.Weixin.SDK.JSSDK
{
    ...
    public class JSAPI
    {
        ...
        /// <returns></returns>
        public static dynamic GetTickect(string AppID)
        {
            ...
            try
            {
                ...
                if (!model.Rev)
                {
                    LogHelper.LogInfo("微信获取token失败,原因:" + model.strRev);
                    return null;
                }
                return model.strTicket;
            }
            catch (Exception ex)
            {
                LogHelper.LogInfo("微信获取token失败,原因:"+ex.Message);
                return null;
            }
