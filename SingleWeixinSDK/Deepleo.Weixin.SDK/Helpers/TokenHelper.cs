...
namespace Deepleo.Weixin.SDK.Helpers
{
    ...
    public class TokenHelper
    {
        ...
        /// <param name="isForceRefresh">是否强制刷新，建议不要频繁刷新</param>
        /// <returns></returns>
        public string GetJSTickect(string AppID,bool isForceRefresh = false)
        {
        //    if (!IsOpenJSTickect) return string.Empty;
        //    if (string.IsNullOrEmpty(_token)) refreshToken();
        //    if (string.IsNullOrEmpty(_jsTickect));
        //    else if (isForceRefresh) refreshJSTickect();
           
            return refreshJSTickect(AppID);
        }
        ...
        private string refreshToken()
        {
            ...
            try
            {
                string  newToken = BasicAPI.GetAccessToken(AppId, AppSecrect);
                return newToken;
                //_token = newToken;
            ...
            {
                Public.LogHelper.LogInfo("微信获取token失败,原因："+ex.Message);
                return null;
                //if (ErrorEvent != null) ErrorEvent(this, new ThreadExceptionEventArgs(ex));
            }
        }

        private string refreshJSTickect(string AppID)
        {
            ...
            try
            {
                string newTickect = JSAPI.GetTickect(AppID);
                return newTickect;
            ...
            {
                Public.LogHelper.LogInfo("微信获取js票据失败,原因：" + ex.Message);
                return null;
            }
